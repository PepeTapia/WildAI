...
header_config = r'--oem 3 --psm 7 digits -c tessedit_char_whitelist="0123456789"'
...
os.makedirs(f'Datasets\{title}',exist_ok=True)
output_csv = os.path.join(base, f'Datasets\{title}\Game{game_num}.csv')
...
with open(output_csv, 'a') as f:
    ...
    #while True: 
        ...
        while "Screen capturing":
            ...
            # Grab screenshot of the monitor, this is a dual monitor approach so we are grabbing monitor 1.
            img = np.array(sct.grab(monitor))


            #Display the picture in grayscale
            gray_array = cv2.cvtColor(img, cv2.COLOR_BGRA2GRAY)

            # ------------------ Time to Text ----------------- #
            
            time_region = gray_array[75:105, 920:1000]

            #Preprocess img for pytesseract

            time_img = pre_img(time_region,200, (150,255))
            ...
            header_score_img = pre_img(header_score, 200, threshold=(100,255), blur=(1,1))
            ...
            start = timer()
            bluesb_region = gray_array[850:1070,740:915]
            ...
            print("whole blue scoreboard process {}".format(end - start))
            #else:
